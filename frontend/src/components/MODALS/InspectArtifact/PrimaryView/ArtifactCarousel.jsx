  ...
  const refreshPictures = async () => {
    try {
      ...
      if (result && result.length > 0) {
        setPictures(result);
        setActiveIndex((prev) => Math.min(prev, result.length - 1));
      }
    } catch (error) {
      ...
      <Carousel interval={null} indicators={false} className="artifact-carousel" activeIndex={activeIndex} onSelect={(selectedIndex) => setActiveIndex(selectedIndex)}>
        {pictures.map((picture) => (
          <Carousel.Item key={picture.pictureid}>
            ...
            <button className="images-options-left images-options" onClick={handleRemovePhoto}> Remove Photo </button>
            <button className="images-options-right images-options" onClick={() => handleInsert()}> Insert Photo </button>
            <Carousel.Caption>
