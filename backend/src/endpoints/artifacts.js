...
endpoint.put('/:id/artifactDetails', async (req, res) => {
    ...
    try {
        ...
        await pool.query(
            'UPDATE Artifacts SET accessionNo = $1, catalogueNo = $2 WHERE artifactID = $4',
            [accessionNo, catalogueNo, id]
        );
        res.status(200).json({ message: 'artifactDetails updated successfully' });
        await pool.query(
            'UPDATE Artifacts SET storageLocation = $1 WHERE artifactID = $2',
            [storageLocation, id]
        );
    } catch (err) {
        console.error('DB ERROR:', err);
